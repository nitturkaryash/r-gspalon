 ...
 */
export function generateStockDetailsExcel(
  products: Product[],
  purchases: Purchase[],
  sales: Sale[],
  ...
  const productIds = new Set<string>(
    [
      ...purchases.map(p => p.product_id).filter((id): id is string => id !== undefined),
      ...sales.map(s => s.product_id).filter((id): id is string => id !== undefined),
      ...consumption.map(c => c.product_id).filter((id): id is string => id !== undefined)
    ]
  );
  ...
  Array.from(productIds).forEach(productId => {
    ...
    const maxTransactions = Math.max(
      ...
      1 // At least one row for each product
    );
...
export function exportInventoryToExcel(
  ...
  consumption: Consumption[],
  balanceStock: BalanceStock[],
  filename: string = 'Salon_Inventory_Transactions.xlsx'
): void {
  // Generate workbook
  const workbook = generateStockDetailsExcel(
    products, 
    purchases, 
    sales, 
    consumption, 
