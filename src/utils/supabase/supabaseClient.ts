    ...
    if (MOCK_DATA_TABLES.includes(table)) {
      ...
      return {
        select: (columns: string) => ({
          order: (column: string, { ascending }: { ascending: boolean } = { ascending: true }) => ({
            eq: (column: string, value: any) => ({
              ...
              then: (callback: Function) => Promise.resolve({ data: mockData.filter(item => item[column] === value), error: null }).then(callback)
            }),
            then: (callback: Function) => Promise.resolve({ data: [...mockData].sort((a, b) => {
              if (ascending) {
          ...
          eq: (column: string, value: any) => ({
            ...
            then: (callback: Function) => Promise.resolve({ data: mockData.filter(item => item[column] === value), error: null }).then(callback),
            order: (column: string, { ascending }: { ascending: boolean } = { ascending: true }) => ({
              then: (callback: Function) => Promise.resolve({ 
                data: [...mockData]
          ...
          then: (callback: Function) => Promise.resolve({ data: mockData, error: null }).then(callback)
        }),
        insert: (items: any[]) => ({
          select: () => ({
            single: () => {
        ...
        delete: () => ({
          ...
          then: (callback: Function) => Promise.resolve({ data: null, error: null }).then(callback)
        }),
        then: (callback: Function) => Promise.resolve({ data: mockData, error: null }).then(callback)
      };
    }
