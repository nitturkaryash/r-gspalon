...
const checkBreakConflict = (stylistId, startTime, endTime, stylists) => {
    // Find the stylist
...
export function useAppointments() {
    ...
    const updateAppointment = useMutation({
        mutationFn: async (updates) => {
            ...
            const index = mockAppointments.findIndex((a) => a.id === updates.id);
            if (index === -1)
    ...
    const updateAppointmentGoogleCalendarId = useMutation({
        mutationFn: async ({ appointmentId, googleCalendarId }) => {
            // Simulate network delay
            await new Promise(resolve => setTimeout(resolve, 500));
