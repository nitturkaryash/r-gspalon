...
export const PAYMENT_METHODS = ['cash', 'credit_card', 'debit_card', 'upi', 'bnpl'];
...
export function usePOS() {
    ...
    const processAppointmentPayment = useMutation({
        mutationFn: async (data) => {
            ...
            await updateClientFromOrder(data.client_name, paidAmount, // Only update with the paid amount
                ...
                try {
                    ...
                    if (localInventoryProducts) {
                        ...
                        productUpdates.forEach(update => {
                            const purchaseIndex = purchases.findIndex((p) => p.id === update.productId);
    ...
    const updateOrderPayment = useMutation({
        mutationFn: async ({ orderId, paymentDetails }) => {
            ...
            saveOrdersToStorage(mockOrders);
            // Update client spending data
            await updateClientFromOrder(order.client_name, safePaymentDetails.amount, safePaymentDetails.payment_method, safePaymentDetails.payment_date);
    ...
    const calculateTotal = (servicePrices, discount = 0, paymentMethod = 'upi', splitPayments) => {
